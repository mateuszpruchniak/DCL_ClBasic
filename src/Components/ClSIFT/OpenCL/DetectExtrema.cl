...
float interp_extremum(__global float* dataIn1, __global float* dataIn2, __global float* dataIn3, int pozX, int pozY, int ImageWidth, int ImageHeight, 
	int intvls, float contrastThreshold, int intvl, float* xi, float* xr, float* xc )
{
	float contr;
	...
	if( (float)ABS( contr ) < (float)contrastThreshold / (float)intvls )
		return 0;
...
*/
 int is_too_edge_like(__global float* dataIn2, int pozX, int pozY, int ImageWidth, int ImageHeight, float curvaturesThreshold )
{
	...
	if( tr * tr / det < (float)( curvaturesThreshold + 1.0 )*( curvaturesThreshold + 1.0 ) / curvaturesThreshold )
		return 0;
	return 1;
}
...
__kernel void KernelDetectProcess(__global float* ucSource, __global float* keys, __global int* number, int OffsetPrev, int Offset, int OffsetNext,
						int ImageWidth, int ImageHeight, float prelim_contrastThreshold, int intvl, int octv)
{
	...
	if( pozX < ImageWidth-SIFT_IMG_BORDER && pozY < ImageHeight-SIFT_IMG_BORDER && pozX > SIFT_IMG_BORDER && pozY > SIFT_IMG_BORDER
		&& ABS(pixel) > prelim_contrastThreshold &&  is_extremum( dataIn1, dataIn2, dataIn3, pozX, pozY, ImageWidth, ImageHeight) == 1  )
	{
...
__kernel void KernelGenDescriptorProcess( __global float* ucSource, int Offset,
						__global int* numberExtrema, __global float* keys,
						int ImageWidth, int ImageHeight, float prelim_contrastThreshold, int intvl, int octv, __global int* number)
 {
